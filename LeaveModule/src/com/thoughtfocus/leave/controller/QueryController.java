...
public class QueryController {
	...
	public ModelAndView userLogin(
			@ModelAttribute("queryBean") QueryBean queryBean) {
	...
	public String leave(
			@ModelAttribute("queryBean") @Valid QueryBean queryBean,
			final BindingResult result, Map<String, Object> map,
		...
		if (result.hasErrors()) {
			...
			try {
				...
				if (null != user) {
					...
					model.addAttribute(QueryConstants.USER_ERROR_MSG,
							QueryConstants.ERROR_MSG_LOGIN_FAILED);
				...
				model.addAttribute(QueryConstants.USER_ERROR_MSG,
						QueryConstants.ERROR_MSG_LOGIN_FAILED_1);
				logger.debug(queryBean.getUserName() + " could not get in");
				logger.error(e.getStackTrace());
				return "UserLogin";
			} catch (Exception e) {
				model.addAttribute(QueryConstants.USER_ERROR_MSG,
						QueryConstants.ERROR_MSG_UNEXPECTED);
				logger.debug(queryBean.getUserName() + " could not get in");
	...
	public String applyLeave(
			@ModelAttribute("leaveBean") @Valid LeaveBean leaveBean,
			final BindingResult result, Map<String, Object> map,
			HttpSession session, HttpServletRequest req, Model model) {
		...
		try {
			...
			User user = (User) session
					.getAttribute(QueryConstants.LOGGED_IN_USER);
			LeaveSummary applyleave = queryManager.applyLeave(leaveBean, user);

			if (null == applyleave) {
				model.addAttribute(QueryConstants.USER_ERROR_MSG,
						QueryConstants.ERROR_MSG_APPLY_LEAVE_FAILED);
			} else {
				model.addAttribute(QueryConstants.USER_ERROR_MSG,
						QueryConstants.ERROR_MSG_APPLY_LEAVE_SUCCESS);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String home(@ModelAttribute("taskBean") @Valid TaskBean taskBean,
			final BindingResult result, Map<String, Object> map,
			HttpSession session, HttpServletRequest req, Model model) {
		List<LeaveType> queryResult = null;
		try {
	...
	public String timesheet(Map<String, Object> map, HttpSession session,
	...
	public String addTask(@ModelAttribute("taskBean") @Valid TaskBean taskBean,
			final BindingResult result, Map<String, Object> map,
			HttpSession session, HttpServletRequest req, Model model) {
		...
		try {
			...
			if (null == addTask) {
				model.addAttribute(QueryConstants.USER_ERROR_MSG,
						QueryConstants.ERROR_MSG_ADD_TASK_FAILED);
			} else {
				model.addAttribute(QueryConstants.USER_ERROR_MSG,
						QueryConstants.ERROR_MSG_ADD_TASK_SUCCESS);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String calendarMonthPrintView(Map<String, Object> map,
			HttpSession session, HttpServletRequest req, Model model) {
